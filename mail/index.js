...
function sendOneMail(obj, callback){
  const createMail = require('./createMail.js');
  const { data: email, subject, body } = obj;
  
  const getAuth = (auth) => {
    const obj = new createMail(auth, email, subject, body);
    let isLastItem = true;
    obj.makeBody(callback, isLastItem);
  }
  mail(getAuth);
