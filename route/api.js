...
const { sendMails, sendOneMail } = require('../mail');
const channel = new SSEChannel();
const csvFilePath = __dirname + "/../csv/contacts.csv";

const {EmailListTable} = require('../db');
...
module.exports = function (app) {
	...
	app.post('/api/testmail', (req, res) => {
		//////  Data, Subject, Body,
		const { testmail: data, subject, body } = req.body;

		if (!data || !subject || !body) return res.json({ message: "Underfined!!!"});

		sendOneMail({ data, subject, body}, (err, result) => {
			if (err) return res.json({ message: '!could not send'});
	...
	app.post('/api/import', uploadCsv, (req, res) => {
		...
		.then((jsonObj)=> {
			EmailListTable.push(jsonObj) // Storing the imported EMAILS to the Database.....
